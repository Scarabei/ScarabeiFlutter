...
public class FlutterView extends SurfaceView implements BinaryMessenger, AccessibilityStateChangeListener {
   ...
   public FlutterView(Context context, AttributeSet attrs) {
      ...
      PlatformPlugin platformPlugin = new PlatformPlugin((Activity)this.getContext());
   ...
   public void onPostResume() {
      ...
      while(var1.hasNext()) {
         ActivityLifecycleListener listener = (ActivityLifecycleListener)var1.next();
   ...
   private void setLocale(Locale locale) {
      this.mFlutterLocalizationChannel.invokeMethod("setLocale", Arrays.asList(new String[]{locale.getLanguage(), locale.getCountry()}));
   }
   ...
   private int getPointerChangeForAction(int maskedAction) {
      return maskedAction == 0?4:(maskedAction == 1?6:(maskedAction == 5?4:(maskedAction == 6?6:(maskedAction == 2?5:(maskedAction == 3?0:-1)))));
   }
   ...
   public boolean onTouchEvent(MotionEvent event) {
      if(!this.isAttached()) {
         ...
         int kPointerDataFieldCount = true;
         int kBytePerField = true;
   ...
   private static native void nativeSetViewportMetrics(long var0, float var2, int var3, int var4, int var5, int var6, int var7, int var8);
   ...
   private void updateViewportMetrics() {
      nativeSetViewportMetrics(this.mNativePlatformView, this.mMetrics.devicePixelRatio, this.mMetrics.physicalWidth, this.mMetrics.physicalHeight, this.mMetrics.physicalPaddingTop, this.mMetrics.physicalPaddingRight, this.mMetrics.physicalPaddingBottom, this.mMetrics.physicalPaddingLeft);
   }

   private void handlePlatformMessage(String channel, byte[] message, final int replyId) {
      BinaryMessenger.BinaryMessageHandler handler = (BinaryMessenger.BinaryMessageHandler)this.mMessageHandlers.get(channel);
      if(handler != null) {
         try {
            ...
            handler.onMessage(buffer, new BinaryMessenger.BinaryReply() {
               ...
               public void reply(ByteBuffer reply) {
                  if(this.done.getAndSet(true)) {
                     ...
                     if(reply == null) {
                        FlutterView.nativeInvokePlatformMessageEmptyResponseCallback(FlutterView.this.mNativePlatformView, replyId);
                     } else {
                        FlutterView.nativeInvokePlatformMessageResponseCallback(FlutterView.this.mNativePlatformView, replyId, reply, reply.position());
                     }
   ...
   private void handlePlatformMessageResponse(int replyId, byte[] reply) {
      BinaryMessenger.BinaryReply callback = (BinaryMessenger.BinaryReply)this.mPendingReplies.remove(Integer.valueOf(replyId));
      if(callback != null) {
         try {
   ...
   private class DiscoveryReceiver extends BroadcastReceiver {
      ...
      public void onReceive(Context context, Intent intent) {
         URI observatoryUri = URI.create(FlutterView.nativeGetObservatoryUri());
